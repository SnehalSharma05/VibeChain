...
import { userAddress } from "./UserDashboard";
import { getArtistStats } from "../songListManager";
const artistStats = {
  totalPlays: 1234567,
  totalEarnings: 45.32,
  monthlyPlays: 123456,
...
export function ArtistDashboard({ onUpload }: ArtistDashboardProps) {
  ...
  const refreshStats = async () => {
    // Fetch updated stats
    const newStats = await getArtistStats(userAddress);
    setStats(newStats);
  };
    ...
    <div className="p-6">
      ...
      <div className="mt-8 bg-gradient-to-tr from-fuchsia-600 to-violet-600 rounded-lg p-6 relative overflow-hidden">
        <div className="relative z-10">
          ...
          <button onClick={handleUpload} className="bg-white text-gray-900 px-6 py-2 rounded-md font-semibold">
            Upload Track
          </button>
        </div>
